...
function ViewAdAppreciation() {
  ...
  async function allfeed() {
    try {
      const response = await fetch(`${import.meta.env.VITE_BASE_URL}/feed/appreciation`);
      const data = await response.json();

      if (data.msg === "Found" && Array.isArray(data.result)) {
        setFeed(data.result); // Set only the array part
        console.log(feed);
        setMsg("View All Appreciations");
      } else {
...
export default ViewAdAppreciation;
